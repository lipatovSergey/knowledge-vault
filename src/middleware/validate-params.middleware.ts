...
function validateParams<Params extends ParamsDictionary>(schema: ZodType<Params>): RequestHandler {
  return (req, _res, next) => {
    const result = schema.safeParse(req.params);
    if (!result.success) {
      return next(new BadRequestError(result.error.issues[0].message, "params"));
    }
